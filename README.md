...
The project includes a `Makefile` at the repo root for automated grading and reproducibility. The pipeline step uses [Nextflow](https://www.nextflow.io/) as the workflow orchestrator.

| Target | Command | Description |
|--------|---------|-------------|
| Setup | `make setup` | Installs Python dependencies, Java, and Nextflow |
| Pipeline | `make pipeline` | Runs the full data pipeline via Nextflow — builds the database and generates all output tables and plots |
| Dashboard | `make dashboard` | Starts the Streamlit dashboard on port 8501 |

**`main.nf`** defines a two-step Nextflow workflow:

1. **`LOAD_DATA`** — runs `load_data.py` to initialize the SQLite schema and load `cell-count.csv` into the database (Part 1)
2. **`GENERATE_OUTPUTS`** — runs `pipeline.py` to produce static output files for Parts 2–4 in the `output/` directory:
...
├── cell_counts.db        ← created by load_data.py / make pipeline
├── load_data.py
├── main.nf
├── Makefile
├── pipeline.py
